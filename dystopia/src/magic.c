...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
   ...
   && (dtype == 2 || dtype == 3))) return;
    ...
    if (ch->spl[dtype] >= 300 + (ch->pcdata->upgrade_level * 5)) return;
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(ch) && IS_AFFECTED(ch, AFF_POLYMORPH) && !IS_CLASS(ch, CLASS_ANGEL) && !IS_CLASS(ch, CLASS_FAE) &&
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    case ITEM_FAITHTOKEN:
        sprintf( buf, "Faith point value is %d.\n\r", obj->value[0] );
        send_to_char( buf, ch );
        break;
	...
	if (itemtype == 4)
	    ...
	    sprintf (buf, "This ancient weapon spews powerful magics.\n\r");
	else if (itemtype == 21)
	...
	if (obj->value[3] == 4)
	    ...
	    sprintf (buf, "This object channels the power of God into its wearer.\n\r");
            ...
            sprintf (buf, "This object channels the power of Lloth into its wearer.\n\r");
	    ...
	    sprintf (buf, "This powerful item spews horrifying magics.\n\r");
	    ...
	    sprintf (buf, "This ancient artifact grants its wearer the power of the third eye.\n\r"); 
            ...
            sprintf( buf, "This cursed item invokes madness.\n\r");
	else
	    sprintf (buf, "This item is bugged...please report it.\n\r");
	if (obj->value[3] > 0)
	    send_to_char( buf, ch );
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  {
    if (!can_see_obj( ch, obj ) || !is_name(arg1, obj->name) || (ch->level < 7 &&
...
void spell_mount( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_CLASS(ch, CLASS_DEMON))
    {
	victim=create_mobile( get_mob_index(MOB_VNUM_DMOUNT));
	...
	return;
        ...
        victim=create_mobile( get_mob_index( MOB_VNUM_SMOUNT ) );
        victim->level = level*2;
        victim->armor = 0 - (10*level);
        victim->hitroll = level*2;
        victim->damroll = level*2;
