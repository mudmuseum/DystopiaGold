...
bool process_output( DESCRIPTOR_DATA *d, bool fPrompt )
{
	...
	ch = d->original ? d->original : d->character;
        ...
        if (IS_SET(ch->act, PLR_VT100) && !IS_NPC(ch))
        {
          sprintf(buf, "%s\e[%d;1H", VT_SAVECURSOR, ch->pcdata->vt100_size);
          write_to_buffer(d, buf, 0);
        }
	if ( IS_SET(ch->act, PLR_BLANK) && !IS_SET(ch->act, PLR_VT100))
	    write_to_buffer( d, "\n\r", 2 );
	...
	else if ( IS_SET(ch->act, PLR_PROMPT) )
	{
		...
		COL_SCALE(exp_str, ch, ch->exp, 1000);
	        sprintf( buf, "#n<[#4%sX#n] [#3?#1H #3?#6M #3?#2V#n]> ",exp_str );
	    }
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	else
	{
	    /* New player */
            ...
            sprintf( buf, "Reminder: If you have chosen a name which isn't suitable, you'll get deleted.\n\r");
            write_to_buffer( d, buf, 0 );
	    sprintf( buf, "Your sure that you want %s engraved on your tombstone (Y/N)? ", argument );
	    write_to_buffer( d, buf, 0 );
	...
	{
  	  if (!ragnarok) sprintf(buf,"#2%s #7enters #RDystopia.#n", ch->name );
          else sprintf(buf,"#2%s #7enters #RDystopia #y(#0Ragnarok#y).#n", ch->name );
          enter_info(buf);
	}
	ch->fight_timer = 0;
        REMOVE_BIT(ch->act, PLR_VT100); // vt100 removed, should make it a tempflag.
  ...
  if ( strcmp( crypt ( argument, ch->pcdata->pwd ), ch->pcdata->pwd ) )
	...
	else
	{
        ...
        players_logged++;
	sprintf(buf,"#7A #Rnew player#7 named #2%s #7enters #RDystopia.#n", ch->name );
	enter_info(buf);
	ch->fight_timer = 0;
...
void bust_a_prompt( DESCRIPTOR_DATA *d )
{
  ...
  while( *str != '\0' )
  {
    ...
    switch( *str )
    {
      ...
      case 'Q' :
        if (!IS_NPC(ch))
          sprintf(buf2, "%d", ch->pcdata->stats[NINJA_POWER]);
        else sprintf(buf2, " ");
        i = buf2; break;
      case 'f' :
      ...
      case 'F' :
       ...
       case 'c' :
         ...
         {
           sprintf(buf2, "%d", ch->pcdata->powers[FAE_ENERGY_TICK]);
         }
         else if (IS_CLASS(ch, CLASS_NINJA) && !IS_NPC(ch))
         {
           if (IS_SET(ch->pcdata->powers[NINJA_BITS], NPOWER_BLOODRAGE))
             sprintf(buf2, "on");
           else sprintf(buf2, "off");
           i = buf2;
           break;
         }
         else sprintf(buf2, "N/A");
         i = buf2; break;
       case 'C' :
         if (IS_CLASS(ch, CLASS_FAE) && !IS_NPC(ch))
         {
           sprintf(buf2, "%d", ch->pcdata->powers[FAE_WILL_TICK]);
         }
         else if (IS_CLASS(ch, CLASS_NINJA) && !IS_NPC(ch))
         {
       ...
       case 'k' :
         ...
         else if (IS_CLASS(ch, CLASS_FAE) && !IS_NPC(ch))
         {
           sprintf(buf2, "%d", ch->pcdata->powers[FAE_PLASMA_TICK]);
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_DRONE) && !IS_NPC(ch))
         {
           sprintf(buf2, "%s", TIME_UP(ch, TIMER_DRONE_DISTORT) ? "ready" : "charging");
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_UNDEAD_KNIGHT) && !IS_NPC(ch))
         {
           sprintf(buf2, "%s", (ch->pcdata->powers[POWER_TICK] == 0) ? "ready" : "charging");
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_HOBBIT) && !IS_NPC(ch))
         {
           if ((obj = get_eq_char(ch, WEAR_SPECIAL)))
           {
             ...
             {
               sprintf(buf2, "%s", flag_string(hobbit_dishes,
                 obj->value[0] + obj->value[1] + obj->value[2] + obj->value[3]));
             }
           }
           else sprintf(buf2, "No Pot");
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_MONK) && !IS_NPC(ch))
         {
           sprintf(buf2, "%s", flag_string(monk_combos, ch->monkcrap));
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_LICH) && !IS_NPC(ch))
         {
           for (tank = ch->in_room->people; tank; tank = tank->next_in_room )
           {
             if (!IS_NPC(tank)) continue;
             if (tank->pIndexData->vnum < MOB_VNUM_FIRE || tank->pIndexData->vnum > MOB_VNUM_CLAY)
               continue;
             if (is_same_group(tank, ch)) members++;
           }
           sprintf(buf2, "%d", members);
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_DROID) && !IS_NPC(ch))
         {
           sprintf(buf2, "%d", ch->pcdata->powers[DROID_AMMO]);
           i = buf2;
           break;
         }
       ...
       case 'K' :
         ...
         {
           sprintf(buf2, "%d", ch->pcdata->powers[DROID_ROCKET]);
           i = buf2;
           break;
         }
         else if (IS_CLASS(ch, CLASS_FAE) && !IS_NPC(ch))
         {
           sprintf(buf2, "%d", ch->pcdata->powers[FAE_MATTER_TICK]);
           i = buf2;
           break;   
         }
         else if (IS_CLASS(ch, CLASS_NINJA) && !IS_NPC(ch))
       ...
       case 'R' :
         if (!IS_NPC(ch) && (IS_CLASS(ch, CLASS_WEREWOLF) ||
              IS_CLASS(ch, CLASS_VAMPIRE) || IS_CLASS(ch, CLASS_NINJA) ||
