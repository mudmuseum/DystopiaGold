...
void do_flash( CHAR_DATA *ch, char *argument )
{
   char       buf[MAX_STRING_LENGTH];
    ...
    sprintf(buf, "%s starts moving with lightning speed.",ch->name);
    act(buf,ch,NULL,NULL,TO_ROOM);
...
void do_vampirearmor( CHAR_DATA *ch, char *argument )
{
  ...
  if (arg[0] == '\0')
  {
    ...
    else if (!str_cmp(arg,"rod" )) vnum = 33056;
    else
    {
    do_vampirearmor(ch,"");
    return;
 ...
 if ( ( victim = get_char_room( ch, arg1 ) ) == NULL )
{
  ...
  if (victim != NULL) set_fighting(ch,victim, TRUE);
...
void do_assassinate( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_CLASS(ch, CLASS_NINJA) && !IS_SET(ch->pcdata->powers[NINJA_SHADOW], NSHADOWS_ASSASSINATE))
  {
    send_to_char("Huh?\n\r", ch);
    return;
  }
    ...
    else if (IS_CLASS(ch, CLASS_NINJA)) chance = 10;
