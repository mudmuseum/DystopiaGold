...
void do_ostat( CHAR_DATA *ch, char *argument )
{
	...
	obj->weight, get_obj_weight( obj ), obj->ownerid );
    send_to_char( buf, ch );
...
void do_force( CHAR_DATA *ch, char *argument )
{
	...
	for ( vch = char_list; vch != NULL; vch = vch_next )
	{
	    ...
	    {
                if (IS_SET(vch->extra, EXTRA_AFK)) afk = TRUE;
                else afk = FALSE;
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
  ...
  if ((fp = fopen(COPYOVER_FILE, "w")) == NULL)
  {
    ...
    log_format ("Could not write to copyover file: %s", COPYOVER_FILE);
  ...
  for (gch = char_list; gch != NULL; gch = gch->next)
  {
    ...
    /* Make sure the player is saved with all his eq */
    if (IS_SET(gch->extra, EXTRA_AFK)) afk = TRUE;
    do_stand(gch, "");
    do_call(gch, "all");
...
void copyover_recover ()
{
	...
	log_format ("Copyover recovery initiated");
	...
	if (!fp) /* there are some descriptors open which will hang forever then ? */
	{
		...
		log_format ("Copyover file not found. Exitting.\n\r");
		exit (1);
	}
