...
{ "Religion",	"Religion messages",		 0,	2,     "all", DEF_EXCLUDE, 21, NULL, FALSE}
};
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	if (IS_NPC(ch)) /* NPC cannot post notes */
        ...
        if (getMight(ch) < 150 && !is_upgrade(ch))
        {
    ...
    { 
      ...
      { /* watch out for the non-portable &%c !!! */
        sprintf (buf,"#G%2d#R> #G%-10s#R [#y%3d#R/#0%3d#R] #G%s#n\n\r",
          count, boards[i].short_name, unread, total_notes(ch, &boards[i]), boards[i].long_name);
        send_to_char (buf,ch);	                    
        count++;
      } /* if has access */
    } /* for each board */
